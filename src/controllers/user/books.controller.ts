...
export const addBook = async (req, res) => {
  ...
  try {
    ...
    const book: BookModel | null = await Book.create({ userId: user.id, bookRef, epubCfi: epubCfi || null, bookName })
...
export const removeBook = async (req, res) => {
  ...
  try {
    ...
    const book: BookModel | null = await Book.findOne({ where: {
      id: bookId,
      userId: user.id
    } })
    if (!book) throw new Error()
...
export const getBooks = async (req, res) => {
  ...
  try {
    ...
    const books: BookModel[] | null = await Book.findAll({where: { userId: user.id }})
...
export const openBook = async (req: any, res: any) => {
  ...
  try {
    const user: UserModel | null = await User.findOne({ where: { username: reqUser?.username } })
    ...
    console.log(e);
    return res.status(500).send('Не поулчается получить данные в данный момент')
  }
}
